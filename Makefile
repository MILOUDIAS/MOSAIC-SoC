...
# NOTE: the glob is restricted to RISC-V *bare-metal* (elf) gcc so an unset or
# mispointed RISCV_XHEEP doesn't silently pick the host compiler (e.g.
# aarch64-...-gcc) and produce a mangled "<host>elf-gcc" toolchain name.
# Set RISCV_XHEEP to your toolchain root (the dir containing bin/) for the
# correct path; it falls back to /bin/riscv32-*-elf-gcc otherwise.
