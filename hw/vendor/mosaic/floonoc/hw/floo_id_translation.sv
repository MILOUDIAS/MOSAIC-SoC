...
module floo_id_translation #(
  ...
  if (RouteCfg.UseIdTable) begin : gen_addr_decoder
    ...
    addr_decode #(
      ...
      // MOSAIC vendoring patch: common_cells 1.38 (vendored in x-heep) asserts
      // NoIndices > 0 and idx < NoIndices; upstream FlooNoC targets 1.39 and
      // only overrides idx_t. Size the index space from the idx type.
      .NoIndices  ( unsigned'(2**$bits(sam_idx_t)) ),
      .addr_t     ( addr_t               ),
      .rule_t     ( addr_rule_t          ),
      .idx_t      ( sam_idx_t            )
    ) i_addr_dst_decode (
    ...
    // MOSAIC vendoring patch: drop the 5th (message) macro argument — the
    // x-heep flattened build resolves `ASSERT to OpenTitan prim_assert's
    // 4-argument macro (upstream FlooNoC targets common_cells 1.39's 5-arg one).
