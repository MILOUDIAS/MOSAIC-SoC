  ...
  always_comb begin
    ...
    case (producer_state_q)
      ...
      PRODUCER_WAIT_FOR_VALID: begin
        ...
        // Read the internal response-pop signal (== producer_resp_o.rvalid, see
        // the assign below) rather than the output port. Functionally identical
        // for synthesis, but reading the output port back here makes Verilator
        // evaluate this FSM with a stale rvalid when a response and the next
        // request coincide, leaving gnt stuck at 0 (the bus then deadlocks on the
        // 2nd transaction). Surfaced only by the full multi-core SoC sim.
        if (fifo_resp_pop) begin
          fifo_req_push = producer_req_i.req && !fifo_req_full;
          producer_resp_o.gnt = !fifo_req_full;
