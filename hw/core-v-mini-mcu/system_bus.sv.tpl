...
% if is_mc:
  // worker instruction-fetch handshake trace (first 40 active cycles per worker
  // hart): shows req/gnt/rvalid/rdata so we can see whether a woken worker's
  // fetch is granted and whether the instruction response comes back.
    ...
    if (!rst_ni) begin
      ...
      for (int h = 1; h < core_v_mini_mcu_pkg::NUM_HARTS; h++) begin  // workers only
        ...
        if ((core_instr_req_i[h].req || core_instr_resp_o[h].rvalid
             || core_data_req_i[h].req || core_data_resp_o[h].rvalid) && dbg_wprints[h] < 40) begin
