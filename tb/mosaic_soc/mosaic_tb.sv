...
module mosaic_tb;
  ...
  always_ff @(posedge clk) if (rst_n && woke1) begin
    if (`CPU.cpu_1_0.trap_o && !fz_trap_seen) begin
      $display("[fz] *** TRAP at pc_r=0x%08x  irq7=%0b  imem_dat=0x%08x",
               `FZPC.pc_r, `CPU.cpu_1_0.irq_i[7], `CPU.cpu_1_0.wb_imem_dat);
      fz_trap_seen <= 1'b1;
    end
    if (`FZPC.pc_r != fz_lastpc && fz_prints < 80) begin
      $display("[fz] pc_r=0x%08x state=%0d stb=%0b ack=%0b trap=%0b irq7=%0b idat=0x%08x",
               `FZPC.pc_r, `FZCT.state_r,
               `CPU.cpu_1_0.wb_imem_stb, `CPU.cpu_1_0.wb_imem_ack,
  ...
  initial forever begin #CLK_HI clk = 1'b0; #CLK_LO clk = 1'b1; end
  ...
  initial begin : load
    ...
    if (!$value$plusargs("firmware=%s", firmware)) begin
      $display("[mosaic_tb] no +firmware"); $finish;
    end
  ...
  task dump;
    ...
    $display("  worker run-latch:     ATLAS=%0d  NANO=%0d",
             `CPU.core_run_1_0, `CPU.core_run_2_0);
    $display("  worker fetch_enable:  ATLAS=%0d  NANO=%0d",
             `CPU.fetch_enable_1_0, `CPU.fetch_enable_2_0);
    $display("  core_sleep[TITAN,ATLAS,NANO] = %0d %0d %0d",
             `CMM.core_sleep[0], `CMM.core_sleep[1], `CMM.core_sleep[2]);
  endtask

  testharness #(
      .JTAG_DPI(0), .USE_EXTERNAL_DEVICE_EXAMPLE(1), .CLK_FREQUENCY(100_000)
  ) testharness_i (
      .clk_i(clk), .rst_ni(rst_n),
      .boot_select_i(boot_sel), .execute_from_flash_i(exec_flash),
      .exit_valid_o(exit_valid), .exit_value_o(exit_value),
      .jtag_tck_i(jtag_tck), .jtag_trst_ni(jtag_trst_n), .jtag_tms_i(jtag_tms),
